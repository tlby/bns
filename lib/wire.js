...
class KEYRecord extends RecordData {
  ...
  toString() {
    ...
    if (isRSA(this.algorithm)) {
      const bits = rsaBits(this.publicKey);
      if (bits) {
        const [n, e] = bits;
        str += ` ; bits = ${n},${e}`;
      }
...
class Option extends Struct {
  ...
  toString() {
    ...
    if (isUnknown) {
      ...
      const size = this.getSize().toString(10);
      body = `\\# ${size} ${body}`;
    }
...
class ECSOption extends OptionData {
  constructor() {
    super();
    this.family = 1;
    this.source = 0;
  ...
  getAddressSize() {
    return Math.ceil(this.source / 8);
  }
  ...
  write(bw) {
    ...
    bw.writeU8(this.source);
    bw.writeU8(this.scope);
    bw.writeBytes(this.address);
  ...
  read(br) {
    ...
    this.source = br.readU8();
    this.scope = br.readU8();
  ...
  toString() {
    let addr;
    if (this.family === 1) {
      addr = IP.toString(util.padRight(this.address, 4));
    } else if (this.family === 2) {
      addr = IP.toString(util.padRight(this.address, 16));
    } else {
      return super.toString();
    }
    return `${addr}/${this.source}/${this.scope}`;
  }
}
...
class EDEOption extends OptionData {
  ...
  getSize() {
    return 1 + sizeString(this.extraText);
  }

  write(bw) {
    bw.writeU16BE(this.infoCode);
    writeRawStringBW(bw, this.extraText);
    return this;
  }

  read(br) {
    this.infoCode = br.readU16BE();
    this.extraText = readRawStringBR(br, br.data.length - br.offset);
    return this;
  }
}
