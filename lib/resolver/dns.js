...
class DNSResolver extends EventEmitter {
  ...
  async exchange(req, servers) {
    ...
    req.id = util.id();
    /* TODO: under extreme concurrency, this could become a bottleneck */
    while (this.pending.has(req.id)) req.id = 0xffff & (req.id + 1);
    req.qr = false;
    ...
    this.socket.send(msg, 0, msg.length, port, host, tcp);
    assert(!this.pending.has(query.id))
    ...
    return new Promise((resolve, reject) => {
      query.resolve = (res) => {
        this.emit('reply', query, null, res);
        resolve(res);
      };
      query.reject = (err) => {
        this.emit('reply', query, err);
        reject(err);
      };
    });
  }
