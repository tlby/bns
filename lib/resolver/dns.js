...
/* provide details about where the error came from */
class ResolverError extends Error {
  addQuery(query) {
    Object.assign(this, query.server);
    return this;
  }
}
...
class DNSResolver extends EventEmitter {
  static ResolverError = ResolverError;
  constructor(options) {
    super();
  ...
  retry(query, rotate, forceTCP, isTimeout) {
    ...
    if (query.attempts >= this.maxAttempts) {
      ...
      if (query.res) {
        query.res.meta = { ...query.server, time: query.time };
        query.resolve(query.res);
      }
      else
        query.reject(new ResolverError('Request timed out.').addQuery(query));
  ...
  handle(msg, rinfo) {
    ...
    try {
      ...
      query.reject(new ResolverError('Encoding error.').addQuery(query));
      return;
    }

    if (!res.qr) {
      this.pending.delete(id);
      query.reject(new ResolverError('Not a response.').addQuery(query));
      return;
    }

    if (!sameQuestion(req, res)) {
      this.pending.delete(id);
      query.reject(new ResolverError('Invalid question.').addQuery(query));
      return;
    }
    ...
    if (this.tcp && res.tc) {
      if (rinfo.tcp) {
        this.pending.delete(id);
        query.reject(new ResolverError('Truncated TCP msg.').addQuery(query));
        return;
      }
    ...
    if (res.opcode !== opcodes.QUERY) {
      this.pending.delete(id);
      query.reject(new ResolverError('Unexpected opcode.').addQuery(query));
      return;
    }
    ...
    if (res.code === codes.FORMERR) {
      this.pending.delete(id);
      query.reject(new ResolverError('Format error.').addQuery(query));
      return;
    }
    ...
    if (!this.verify(msg, host, port)) {
      this.pending.delete(id);
      query.reject(new ResolverError('Could not verify response.').addQuery(query));
      return;
    }

    this.pending.delete(id);

    res.meta = { ...query.server, time: query.time };
    query.resolve(res);
  }
