...
function readOption(wire, iter, code) {
  ...
  // Special case.
  if (op.code === options.ECS) {
    const comment = parseECS(od, iter, '');
    return [op, comment];
  }
...
function parseECS(op, iter, file) {
  ...
  op.source = 0;
  op.scope = 0;
  op.address = Buffer.from('');

  const tok = expect(iter, tokens.STRING, file);
  const [addr, source, scope] = tok.string.split('/');
  op.source = util.parseU8(source);
  op.scope = util.parseU8(scope);
  const pack = IP.toBuffer(addr);
  const size = Math.ceil(op.source / 8);
  switch (IP.isIP(pack)) {
    case 4:
      op.family = 1;
      op.address = pack.subarray(12, 12 + size);
      break;
    case 6:
      op.family = 2;
      op.address = pack.subarray(0, size);
      break;
    default:
      throw new ParseError(`invalid ECS.`, tok, file);
  }
  return tok.comment;
}
