...
encoding.readIP = function readIP(br, size) {
  const s = IP.readBR(br, size);
  /* undo the IPv6 to IPv4 reduction at
   * https://github.com/bcoin-org/binet/blob/9f6a82025db7722abce554749147a832348f5b07/lib/ip.js#L147-L149
   * because AAAA records with IPv4 results are cumbersome */
  if (size === 16 && !s.includes(':')) return `::ffff:${s}`;
  return s;
};
